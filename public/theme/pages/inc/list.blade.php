<li>
    ...
    <div class="desc">
        ...
        <span>
            <a href="{{action('WelcomeController@showArticle',checkItem($item->translate_slug,$item->slug))}}" onClick="return ajaxRoute('{{action("WelcomeController@showArticle",checkItem($item->translate_slug,$item->slug))}}','{{checkItem($item->translate_slug,$item->slug)}}')">{{recordDesc($item->head,$item->body,136,recordTitle($item->frontpage_title,$item->title),60)}}</a>
        </span>
        <br>
        <div class="time-author">
            @if($item->author)<span><a href="{{action('WelcomeController@newsAuthor',$item->author)}}" onClick="return ajaxRoute('{{action("WelcomeController@newsAuthor",$item->author)}}','{{$item->author}}')">{{$item->author}}</a></span>@endif<span>{{date('m.d.Y',strtotime($item->published_at))}}</span>
        </div>
    </div>
</li>
